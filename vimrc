...
map <Leader>rt :!bundle list --paths=true \| xargs ctags --ruby-types= --extra=+f --exclude=.git --exclude=log -R *<CR><CR>
...
"enable copying formatted text from within vim as HTML to for example, paste
"in an email
vnoremap _y :TOhtml<CR>:sav! temp.html<CR>:bd<CR>:!open temp.html<CR><CR>:sleep 1<CR>:!rm temp.html<CR><CR>
...
au BufNewFile,BufRead {*.hamlbars,*.hbs}                                set ft=haml
...
nnoremap ,F :let word=expand(""):vsp:wincmd w:exec("tag ". word)
